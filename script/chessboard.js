var chessBoard = angular
			...
			for (rocount = 8; rocount > 0; rocount--) {
				...
				for (var colcount = 1; colcount <= 8; colcount++) {
					row.cols[row.cols.length] = {
						'tdClass' : '',
						'divClass' : divClass,
						'id' : colidarray[colcount - 1] + rocount,
						'value' : ''
					};
		...
		.factory(
					...
					var fns = {
						updateCurrentScreenShot : function($scope) {
							...
							angular.forEach(pieceArray, function(piece) {
								$scope.screenshot["white" + piece] = fns
										.findPiece("white" + piece);
								$scope.screenshot["black" + piece] = fns
										.findPiece("black" + piece);
							});
						},
						playMove : function($scope, suggestedMove) {
							if (suggestedMove == "O-O") {
								...
								if (moveTypeSymbol == "x") {
									if ((piecetype == "P")
											&& (oldcellId.split('')[0] != newcellId
													.split('')[0])
											&& (fns.isEmptyCell(newCell))) {
										var row;
										if ($scope.nextmoveby == "black") {
											row = 4;
										} else {
										...
										var enpassCell = $("#"
												+ newcellId.split('')[0] + row);
										enpassCell
												.removeClass(fns
														.getPieceType(enpassCell)
														+ "_"
														+ fns
																.getCellColour(enpassCell));
									} else {
										newCell.removeClass(fns
												.getPieceType(newCell)
												+ "_"
												+ fns.getCellColour(newCell));
									}
								}
								newCell.addClass(piece + "_" + newCellColour);
								currentCell.removeClass(piece + "_"
										+ fns.getCellColour(currentCell));
							}
						},
						restoreFromScreenshot : function($scope) {
							var restorePiece = function(piece) {
								$("." + piece + "_black").removeClass(
										piece + "_black");
								$("." + piece + "_white").removeClass(
										piece + "_white");
								for (var i = 0; i < $scope.screenshot[piece].length; i++) {
									$scope.screenshot[piece][i]
											.addClass(piece
													+ "_"
													+ fns
															.getCellColour($scope.screenshot[piece][i]));
								}
							}
							var pieceArray = [ "pawn", "rook", "knight",
									"bishop", "queen", "king" ];
						...
						isValidMoveRep : function($scope, suggestedMove) {
							var isMovRepValid = false;
							if (suggestedMove == "O-O") {
								if (($scope.nextmoveby == "black")
										&& ($("#h8")
												.hasClass("blackrook_black") && ($("#e8")
												.hasClass("blackking_white")))) {
								// TODO No check at e8 f8 g8
								} else if ($("#h1").hasClass("whiterook_white")
										&& $("#e1").hasClass("whiteking_black")) {
								//TODO No check at e1 f1 g1
								}
								isMovRepValid = true;
							} else if (suggestedMove == "O-O-O") {
								if (($scope.nextmoveby == "black")
										&& ($("#a8")
												.hasClass("blackrook_black") && ($("#e8")
												.hasClass("blackking_white")))) {
								// TODO No check at c8 d8 e8
								} else if ($("#a1").hasClass("whiterook_white")
										&& $("#e1").hasClass("whiteking_black")) {
								//TODO No check at c1 d1 e1
								}
								// TODO check if this is vald
								isMovRepValid = true;
							} else {
								try {
									var moveparts = suggestedMove.split(" ");
							...
							return isMovRepValid;
						},
						isMoveLegal : function($scope, suggestedMove) {
							var moveLegal = false;
							if (fns.isValidMoveRep($scope, suggestedMove)) {
								fns.updateCurrentScreenShot($scope);
								fns.playMove($scope, suggestedMove);
								var stillunderCheck = fns.isKingUnderCheck(
										$scope, $scope.nextmoveby);
								fns.restoreFromScreenshot($scope);
								moveLegal = !stillunderCheck;
							}
						...
						findPiece : function(piece) {
							...
							if ($("." + piece + "_white").length > 0) {
								cellArray[cellArray.length] = $("." + piece
										+ "_white");
							}
						...
						isKingUnderCheck : function($scope, kingColour) {
							var isChecked = false;
							var kingCellArray = fns.findPiece(kingColour
									+ "king");
							var kingCell = kingCellArray[0];
							var opponentColour = fns
									.getOpponentColour(kingColour);
							var checkByPiece = function(piece, opponentColour,
									kingCell) {
								var isChecked = false;
								var cellArray = fns.findPiece(opponentColour
										+ piece);
								angular.forEach(cellArray, function(cell) {
									if (fns.getPossibleMovesByPieceOnCell(
											$scope, cell).indexOf(
											kingCell.attr('id')) > -1) {
										isChecked = true;
									}
							...
							angular.forEach(pieceArray, function(piece) {
								if (!isChecked) {
									isChecked = checkByPiece(piece,
											opponentColour, kingCell);
								}
							});
							return isChecked;
						},
						...
						getPieceType : function(cell) {
							...
							var pieceArray = [ "pawn", "rook", "knight",
									"bishop", "queen", "king" ];
							var checkByPiece = function(cell, pieceToBeChecked,
									color) {
								var piece = "";
								if (cell.hasClass("white" + pieceToBeChecked
										+ "_" + color)) {
									piece = "white" + pieceToBeChecked;
								} else if (cell.hasClass("black"
										+ pieceToBeChecked + "_" + color)) {
							...
							angular.forEach(pieceArray, function(pieceInArray) {
								if (piece == "") {
									piece = checkByPiece(cell, pieceInArray,
											color);
								}
							});
							return piece;
						...
						isLegalCell : function(cell) {
							return cell.hasClass("blackcell")
									|| cell.hasClass("whitecell");
						},
						...
						selectCell : function(cell) {
							if (!fns.isEmptyCell(cell)) {
								...
								$(".possiblemovecell").removeClass(
										"possiblemovecell");
								cell.addClass("active");
							}
						},
						getMoveRepresentation : function($scope, piece,
								oldCell, newCell, specialCase) {
							var pieceSymbol = fns.getPieceSymbol(piece);
							var moverepresentation;
							var casesymbol = "-";
							if ((specialCase == "capture")
									|| (!fns.isEmptyCell(newCell))) {
								casesymbol = "x";
							}
							if (pieceSymbol == "P") {
								...
								if ((oldCellRow[0] != newCellRow[0])
										&& (fns.isEmptyCell(newCell))) {
									casesymbol = "x";
								}
							}
							moverepresentation = pieceSymbol + " "
									+ oldCell.attr("id") + " " + casesymbol
									+ " " + newCell.attr('id');
							if ((moverepresentation == "K e1 - g1")
									|| (moverepresentation == "K e8 - g8")) {
								moverepresentation = "O-O";
							} else if ((moverepresentation == "K e1 - c1")
									|| (moverepresentation == "K e8 - c8")) {
								moverepresentation = "O-O-O";
							}
							return moverepresentation;
						},
						logMove : function($scope, moverepresentation) {
							if ($scope.nextmoveby == "white") {
								$("#logtable")
										.append(
												"<tr><td class='whitelog'>"
														+ moverepresentation
														+ "</td><td class='blacklog lastBlackLog'></td></tr>");
							} else {
						...
						moveTo : function($scope, cell) {
							...
							if (fns.isPawnPromotion(piece, cell)) {
								...
								if (!fns.isEmptyCell(cell)) {
									cell.removeClass(fns.getPieceType(cell)
											+ "_" + newCellColour);
									isCapture = true;
									specialCase = "capture";
								}
							}
							var moverep = fns.getMoveRepresentation($scope,
									piece, currentCell, cell, specialCase);
							fns.playMoveWithFrills($scope, moverep);
						},
						playMoveWithFrills : function($scope, moverep) {
							if (fns.isMoveLegal($scope, moverep)) {
								fns.logMove($scope, moverep);
								fns.playMove($scope, moverep);
								$(".active").removeClass("active");
								$(".possiblemovecell").removeClass(
										"possiblemovecell");
								$scope.nextmoveby = fns
										.getOpponentColour($scope.nextmoveby);
								if (fns.isKingUnderCheck($scope,
										$scope.nextmoveby)) {
									$("#warning").text("Check!!");
								} else {
									$("#warning").text("");
							...
							fns.playNextMoveIfRequired($scope);
						},
						playNextMoveIfRequired : function($scope) {
							var move = "";
							if ((($scope.nextmoveby == $scope.player1.piececolour) && ($scope.player1.auto == "true"))
									|| (($scope.nextmoveby == $scope.player2.piececolour) && ($scope.player2.auto == "true"))) {
								fns.playMoveWithFrills($scope, fns
										.getNextMove($scope));
							}
						},
						getNextMove : function($scope) {
							fns.updateCurrentScreenShot($scope);
							console
									.log("jj------------------------------------")
						...
						highlightPossibleMoves : function($scope, cell) {
							var highlightCellArray = fns
									.getPossibleMovesByPieceOnCell($scope, cell);
							for (i = 0; i < highlightCellArray.length; i++) {
								$("#" + highlightCellArray[i]).addClass(
										"possiblemovecell");
							}
						},
						getPossibleMovesByPieceOnCell : function($scope, cell) {
							...
							possibleMovesCellArray = movesMethods[piece](
									$scope, cell, fns.getPieceColour(piece));
							if (($scope.nextmoveby == fns.getPieceColour(piece))) {
								var filteredMoves = new Array();
								for (var i = 0; i < possibleMovesCellArray.length; i++) {
									var newcell = $("#"
											+ possibleMovesCellArray[i]);
									var moverep = fns.getMoveRepresentation(
											$scope, piece, cell, newcell, "");
									if (fns.isMoveLegal($scope, moverep)) {
										filteredMoves[filteredMoves.length] = possibleMovesCellArray[i];
									}
								}
								if (piece == "whiteking") {
									// if current position of king is original
									// (unmoved) and casteling wali move is
									// there, but middle pe check hai, to remove
									// castelign wali cell also
									if (cell.attr("id") == "e1") {
										if ((possibleMovesCellArray
												.indexOf("f1") > -1)
												&& (possibleMovesCellArray
														.indexOf("g1") > -1)
												&& (filteredMoves.indexOf("f1") < 0)) {
											var index = filteredMoves
													.indexOf("g1");
											if (index > -1) {
						...
						getPawnMoves_moves : function($scope, currentcell,
								pawnColour) {
							...
							// seedha chalne wale
							if (pawnColour == "black") {
								var cellId = cellPosition[0]
										+ (parseInt(cellPosition[1]) - 1);
								if (fns.isEmptyCell($("#" + cellId))) {

									possibleMovesArray[0] = getMoveRepresentation(
											$scope, piece, oldCell, $("#"
													+ cellId), specialCase);
								}
								if (cellPosition[1] == 7) {
									var cellId2 = cellPosition[0]
											+ (parseInt(cellPosition[1]) - 2);
									if (fns.isEmptyCell($("#" + cellId))
											&& fns
													.isEmptyCell($("#"
															+ cellId2))) {
										possibleMovesArray[1] = getMoveRepresentation(
												$scope, piece, oldCell, $("#"
														+ cellId2), specialCase);
									}
								}
								// kaatne wale
								var adjacentcols = fns
										.getAdjacentCols(cellPosition[0]);
								for (var i = 0; i < adjacentcols.length; i++) {
									if (fns.cellHasTeamPiece($("#"
											+ adjacentcols[i]
											+ (parseInt(cellPosition[1]) - 1)),
											"white")) {
										possibleMovesArray[possibleMovesArray.length] = getMoveRepresentation(
												$scope,
												piece,
												oldCell,
												$("#"
						...
						getPawnMoves : function($scope, currentcell, pawnColour) {
							...
							if (pawnColour == "black") {
								...
								// en pas on
								// if current pawn is on fifth line, and in last
								// move a neighbour moved 2 cells
								if (cellPosition[1] == 4) {
									...
									var neighbour = fns
											.getAdjacentCols(cellPosition[0]);
									// for all neighnours
									var move = "P " + neighbour[0] + "2 - "
											+ neighbour[0] + "4";
									if (move == lastMove.textContent) {
										possibleMovesArray[possibleMovesArray.length] = neighbour[0]
												+ "3";
									}
									var move = "P " + neighbour[1] + "2 - "
											+ neighbour[1] + "4";
									if (move == lastMove.textContent) {
										possibleMovesArray[possibleMovesArray.length] = neighbour[1]
												+ "3";
									}
								}
							} else if (pawnColour == "white") {
						...
						getWestCols : function(col) {
							...
							return cols.slice(cols.indexOf(col) + 1,
									cols.length);
						},
						...
						cellHasTeamPiece : function(cell, colour) {
							if ("white" == colour) {
								return cell
										.is(".whitepawn_white,.whiterook_white,.whiteknight_white,.whitebishop_white,.whiteking_white,.whitequeen_white,.whitepawn_black,.whiterook_black,.whiteknight_black,.whitebishop_black,.whiteking_black,.whitequeen_black");
							} else if ("black" == colour) {
								return cell
										.is(".blackpawn_white,.blackrook_white,.blackknight_white,.blackbishop_white,.blackking_white,.blackqueen_white,.blackpawn_black,.blackrook_black,.blackknight_black,.blackbishop_black,.blackking_black,.blackqueen_black");
							}
						...
						getRookMoves : function($scope, currentcell, colour) {
							...
							var getverticalMoves = function(method) {
								...
								while (fns.isLegalCell($("#" + cellId))) {
									if (fns.isEmptyCell($("#" + cellId))) {
										...
										if (fns.getPieceColour(fns
												.getPieceType($("#" + cellId))) != colour) {
											possibleMovesArray[possibleMovesArray.length] = cellId;
										}
										break;
									}
							...
							$.merge(
											possibleMovesArray,
											getverticalMoves(function(
													cellPosition, i) {
												return cellPosition[0]
														+ (parseInt(cellPosition[1]) + i);
											}));
							$.merge(
											possibleMovesArray,
											getverticalMoves(function(
							...
							$.merge(possibleMovesArray, getHorizontalMoves(fns
									.getWestCols(cellPosition[0])));
							$.merge(possibleMovesArray, getHorizontalMoves(fns
									.getEastCols(cellPosition[0])));
						...
						getKnightMoves : function($scope, currentcell, colour) {
							...
							var isCellValid = function(cellId, colour) {
								if (fns.isLegalCell($("#" + cellId))
										&& (fns.isEmptyCell($("#" + cellId)) || fns
												.getPieceColour(fns
														.getPieceType($("#"
																+ cellId))) != colour)) {
									return true;
							...
							allMovesArray[allMovesArray.length] = fns
									.getWestCols(cellPosition[0])[0]
									+ (parseInt(cellPosition[1]) + 2);
							allMovesArray[allMovesArray.length] = fns
									.getEastCols(cellPosition[0])[0]
									+ (parseInt(cellPosition[1]) + 2);
							allMovesArray[allMovesArray.length] = fns
									.getWestCols(cellPosition[0])[0]
									+ (parseInt(cellPosition[1]) - 2);
							allMovesArray[allMovesArray.length] = fns
									...
									+ (parseInt(cellPosition[1]) - 2);
							allMovesArray[allMovesArray.length] = fns
									.getEastCols(cellPosition[0])[1]
									+ (parseInt(cellPosition[1]) - 1);
							allMovesArray[allMovesArray.length] = fns
									.getWestCols(cellPosition[0])[1]
									+ (parseInt(cellPosition[1]) - 1);
							allMovesArray[allMovesArray.length] = fns
									.getEastCols(cellPosition[0])[1]
									+ (parseInt(cellPosition[1]) + 1);
						...
						getBishopMoves : function($scope, cell, colour) {
							...
							var checkContinuousCells = function(
									cellIdentifyFunction) {
								var possibleMovesArray = [];
								var i = 1;
							...
							cellFunctions[cellFunctions.length] = function(i) {
								return fns.getEastCols(cellPosition[0])[i - 1]
										+ (parseInt(cellPosition[1]) + i);
							};
							cellFunctions[cellFunctions.length] = function(i) {
								return fns.getWestCols(cellPosition[0])[i - 1]
										+ (parseInt(cellPosition[1]) + i);
							};
							cellFunctions[cellFunctions.length] = function(i) {
								return fns.getEastCols(cellPosition[0])[i - 1]
							...
							angular.forEach(cellFunctions, function(method) {
								$.merge(possibleMovesArray,
										checkContinuousCells(method));
							});
							return possibleMovesArray;
						},
						...
						getQueenMoves : function($scope, cell, colour) {
							var possibleMovesArray = fns.getBishopMoves($scope,
									cell, colour);
							angular
									.forEach(
											fns.getRookMoves($scope, cell,
													colour),
											function(move) {
						...
						getKingMoves : function($scope, currentcell, colour) {
							...
							cellIdArray[cellIdArray.length] = fns
									.getEastCols(cellPosition[0])[0]
									+ (parseInt(cellPosition[1]) + 1);
							cellIdArray[cellIdArray.length] = cellPosition[0]
									+ (parseInt(cellPosition[1]) + 1);
							cellIdArray[cellIdArray.length] = fns
									.getWestCols(cellPosition[0])[0]
									+ (parseInt(cellPosition[1]) + 1);
							...
							// if king has not moved, and rook has not moved,
							// and there is no check on the path, allow
							// casteling
							var logtobeChecked = ".blacklog";
							...
							for (var i = 0; i < $(logtobeChecked).length; i++) {
								if (($(logtobeChecked)[i].textContent
										.indexOf("K") == 0)
										|| ($(logtobeChecked)[i].textContent
												.indexOf("O") == 0)) {
									hasKingMoved = true;
									break;
								}
							...
							if (!((hasKingMoved) || (hasRook1Moved) || ($(
									"#warning").text() == "Check!!"))) {
								var cell1ToBeChecked = $("#d" + row);
								var cell2ToBeChecked = $("#c" + row);

								if (fns.isEmptyCell(cell1ToBeChecked)
										&& fns.isEmptyCell(cell2ToBeChecked)) {
									// If there is a check on c/d don't add this
									// cell: Filtered out in next step
									possibleMovesArray[possibleMovesArray.length] = 'c'
											+ row;
								}
							}
							if (!((hasKingMoved) || (hasRook2Moved) || ($(
									"#warning").text() == "Check!!"))) {
								var cell1ToBeChecked = $("#f" + row);
								var cell2ToBeChecked = $("#g" + row);
								if (fns.isEmptyCell(cell1ToBeChecked)
										&& fns.isEmptyCell(cell2ToBeChecked)) {
									// If there is a check on f/g don't add this
									// cell : filtered out in next step
									possibleMovesArray[possibleMovesArray.length] = 'g'
											+ row;
								}
							}
						...
						isPawnPromotion : function(piece, cell) {
							if (((piece == "whitepawn") && (cell.attr("id")
									.split("")[1] == 8))
									|| ((piece == "blackpawn") && (cell.attr(
											"id").split("")[1] == 1))) {
								return true;
							} else {
								return false;
						...
						hasMoveResultedInCheck : function($scope, move) {
							// TODO improve logic for shade check. e.g. if
							// opponent knight is in path of opp queen and our
							// king, opponent moves knight, should result in
							// check
							var cellArray = fns.getPossibleMovesByPieceOnCell(
									$scope, cell);
							for (var i = 0; i < cellArray.length; i++) {
								var piece = fns.getPieceType($("#"
										+ cellArray[i]));
				...
				function($scope, chessBoardService) {
					...
					$scope.alertval = function(val) {
						...
						if (thisCell.is(".possiblemovecell")) {
							...
							if ($scope.nextmoveby == chessBoardService
									.getPieceColour(chessBoardService
											.getPieceType(thisCell))) {
								$(".active").removeClass("active");
								$(".possiblemovecell").removeClass(
										"possiblemovecell");
								thisCell.addClass("active");
								chessBoardService.highlightPossibleMoves(
										$scope, thisCell);
							}
						}
					}
